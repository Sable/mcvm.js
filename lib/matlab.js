...
function matlabArrayCopyOnWriteSet (v, x, y, z) {
  if (this._array.refs.length > 1) {
    this._array = new Float64Array(this._array)
  }
  delete this.set
  return this.set(v, x, y, z)
}
...
matlab.Array = function (a, b, c) {
  ...
  // Copy 'a' with Copy-on-Write behaviour
  if (matlab.Array.prototype.isPrototypeOf(a)) {
    if (!this._array.hasOwnProperty('refs')) {
      this._array.refs = new WeakSet()
    }
    this._array.refs.add(this)
    this._array.refs.add(a)
    this._array = a._array
    this._size = arrayCopy(a._size)
    this.set = matlabArrayCopyOnWriteSet
    a.set = matlabArrayCopyOnWriteSet
    return this
  }
...
matlab.Array.prototype = {
  ...
  get_nbc_1: function (x) {
    var d1 = x - 1
    return this._array[d1]
  },
  get_nbc_2: function (x, y) {
    var d1 = x - 1
    var d2 = y - 1
    return this._array[(d2 * this._size[0]) + d1]
  },
