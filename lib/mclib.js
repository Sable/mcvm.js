...
_McLib._gen_arithmetic_binop = function (op, name) {
  name = name || op
  return new Function(['a', 'b'],
    "if (typeof a === 'number' && typeof b === 'number') {\n" +
    '    return a ' + op + ' b\n' +
    '} else if (!a.getSize().equals(b.getSize())) {\n' +
    "    throw new Error('" +
    'Error using ' + name + '\\nMatrix dimensions must agree.\\n' +
    "size(A) == ' + a.getSize().toString() + ', size(B) == ' + b.getSize().toString() + '\\n')\n" +
    '} else {\n' +
    '    return a.map(function (x,i) { return x ' + op + ' b.get(i) }) \n' +
    '}')
}

_McLib._gen_logical_binop = function (op, name) {
  name = name || op
  return new Function(['a', 'b'],
    "if (typeof a === 'number' && typeof b === 'number') {\n" +
    '    return a ' + op + ' b ? 1 : 0\n' +
    ...
    "    throw new Error('" +
    'Error using ' + name + '\\nMatrix dimensions must agree.\\n' +
    "size(A) == ' + a.getSize() + ', size(B) == ' + b.getSize() + '\\n')\n" +
    '} else {\n' +
    '    return a.map(function (x,i) { return x ' + op + ' b.get(i) ? 1 : 0 }) \n' +
    '}')
}
...
_McLib.uminus = function (x) {
  if (typeof x === 'number') {
    return -x
  } else {
    throw new Error('Unimplemented unary minus for non-scalars')
  }
}

_McLib.all = function (a, dim) {
  if (dim === undefined) {
    var allNonZero = true
    ...
    throw new Error('all: Unimplemented dimension argument ' + dim)
  }
}

_McLib.any = function (a, dim) {
  if (dim === undefined) {
...
_McLib.size = function (a, dim) {
  if (dim === undefined) {
    return a.getSize()
  } else {
    throw new Error('size: Unimplemented dimension argument ' + dim)
  }
}
